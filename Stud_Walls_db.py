...
def Size_Studs(stud: Section, spacing: float, combo: str, load: float, load_dict: dict):
    ...
    Pr = {
        'Width': O86.CL6_5_6_2_3(stud,stud.Lu['Width']/(spacing),**k_factors),
        'Depth': O86.CL6_5_6_2_3(stud,stud.Lu['Depth']/(spacing),**k_factors)
    }
...
for level in range(n_floors,0, -1):
    ...
    #loop over studs and get results for each type/size of stud
    for stud in studs:
        ...
        for combo, load in load_combo_dict.items():
            ...
            while DC >= DC_max:
                ...
                results_dict[combo] = Size_Studs(stud, spacing, combo, load, load_dict)
                if stud.Plys > 3: # if max number of plys is reached (3) decrease spacing
                    stud.Plys = 1 # reset plys
                    i += 1
                    spacing = spacings[i]
                    results_dict[combo] = Size_Studs(stud, spacing, combo, load, load_dict)
                else: # nothing works reset all values and display error message
                    break
                DC = results_dict[combo]['DC']
        ...
        print(f"{governing_lc} - ({result['stud'].Plys}){result['stud'].Name} @ {result['spacing']}")
